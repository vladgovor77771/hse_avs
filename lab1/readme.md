...
Все тесты проходятся. Запуск тестирования через `run_tests.sh` или же `make test`.
...
Сборка осуществляется через вызов `make`, который создает необходимые `.o` файлы для, а также сам исполняемый файл `app.out` в директории `bin`.
